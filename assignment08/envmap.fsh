...
void main() {
   ...
   if (uUseCubeMapping) {
     ...
     vec3 shift=normalize(vec3(reflectionVector.x,reflectionVector.y,reflectionVector.z+1));
//     float p=length(shift);


//     vec2 sphereCoord = vec2(reflectionVector.x/(2*p)+0.5,reflectionVector.y/(2*p)+0.5);
      vec2 sphereCoord=vec2(shift.x/2+0.5,shift.y/2+0.5);
   ...
   if (uEnvironmentOnly) {
     ...
     vec3 fragColor = lighingTerm*0.9+reflectionColor*0.1; //Replace this line
     oFragColor = vec4(fragColor, 1.0f);
   }
